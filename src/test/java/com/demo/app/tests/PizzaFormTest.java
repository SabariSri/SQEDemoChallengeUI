...
public class PizzaFormTest extends TestBase {
    ...
    public void TC01_place_order(Map<String, String> data) {
        getReport().stepInfo(TestDataConstants.OtherConstants.REPORT_STEP_DATA + data.toString());
        pizzaOrderPage.chooseMenu(data);
        String expectedTotalCost = getExpectedTotalCost(data);
        pizzaOrderPage.addPickupInfo(data);
        pizzaOrderPage.selectPayment(data.get(TestDataConstants.ColumnName.PAYMENT.toString()));
    ...
    private String getExpectedTotalCost(Map<String, String> data) {
        String pizza1Text = pizzaOrderPage.getPizza1Text(data.get(TestDataConstants.ColumnName.PIZZA1.toString()));
        return String.valueOf((Double.parseDouble(pizza1Text.split("\\$")[1].trim()) *
                Integer.parseInt(data.get(TestDataConstants.ColumnName.QUANTITY.toString()))));
    }
}
